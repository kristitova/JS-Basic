...
<div class="product">
    ...
    <div>${здесь_цена}</div>
    <a href="https://example.com/products/${здесь_id_товара}">Подробнее</a>
</div>
*/
...
function showCategory(category) {
    ...
    switch(category) {
        case ('phones'):
            for (let product = 0; product <= (products['phones'].length-1); product++) {
                getProductMarkup(products['phones'][product]);
            }
            break;
        case ('tablets'):
            for (let product = 0; product <= (products['tablets'].length-1); product++) {
                getProductMarkup(products['tablets'][product]);
            }
            break;
        case ('tv'):
            for (let product = 0; product <= (products['tv'].length-1); product++) {
                getProductMarkup(products['tv'][product]);
            }  
            break;
...
function getProductMarkup(product) {
    products_class.insertAdjacentHTML('beforeend', 
        `<div class='product'>
...
let products_class = document.querySelector('.products');
